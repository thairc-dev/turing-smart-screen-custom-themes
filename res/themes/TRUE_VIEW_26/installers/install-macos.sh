...
INSTALL_DIR="${HOME}/Library/Application Support/TRUEVIEW26"
PLIST="${HOME}/Library/LaunchAgents/com.trueview26.display.plist"
LEGACY_PLIST="${HOME}/Library/LaunchAgents/com.antigravity.trueview26.plist"
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.trueview26.display</string>
  <key>ProgramArguments</key>
  <array>
    <string>${INSTALL_DIR}/.venv/bin/python</string>
    <string>-m</string><string>trueview26</string>
...
for other_label in com.nexus26.display com.nexusminimal.display; do
  ...
  if [[ -f "${other_plist}" ]]; then
    /bin/mv -f "${other_plist}" "${other_plist}.disabled-by-trueview26"
  fi
done
if [[ -f "${LEGACY_PLIST}" ]]; then
  /bin/launchctl bootout "gui/$(id -u)/com.antigravity.trueview26" 2>/dev/null || true
...
if [[ "${loaded}" != true ]]; then
  echo "Could not register com.trueview26.display after 5 attempts." >&2
  exit 1
fi
echo "TRUE VIEW 26 installed and started from ${INSTALL_DIR}"
