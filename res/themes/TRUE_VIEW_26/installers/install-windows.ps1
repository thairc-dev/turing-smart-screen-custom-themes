...
$InstallDir = Join-Path $env:LOCALAPPDATA "TRUEVIEW26"
...
$Action = New-ScheduledTaskAction `
    -Execute $VenvPython `
    -Argument "-m trueview26 --config `"$InstallDir\config.yaml`"" `
...
Register-ScheduledTask -TaskName "TRUEVIEW26 Display" -Action $Action -Trigger $Trigger -Settings $Settings -Force | Out-Null
Start-ScheduledTask -TaskName "TRUEVIEW26 Display"
Write-Host "TRUE VIEW 26 installed and started from $InstallDir"
