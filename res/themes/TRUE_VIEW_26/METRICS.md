...
TRUE VIEW 26 renders only collected operating-system or sensor values. It does
...
| GPU load | `macmon` on Apple Silicon | Genuine GPU active residency. Displays `N/A` when no supported sensor backend exists. |
| Load 1m | `psutil.getloadavg` | Operating-system one-minute load average. This is scheduler load, not CPU percent. |
| Processes | `psutil.pids()` | Total process count, labelled `TOTAL` rather than implying every process is running. |
