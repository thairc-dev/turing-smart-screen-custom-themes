...
INSTALL_DIR="${HOME}/Library/Application Support/NEXUS26"
PLIST="${HOME}/Library/LaunchAgents/com.nexus26.display.plist"
LEGACY_PLIST="${HOME}/Library/LaunchAgents/com.antigravity.nexus26.plist"
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.nexus26.display</string>
  <key>ProgramArguments</key>
  <array>
    <string>${INSTALL_DIR}/.venv/bin/python</string>
    <string>-m</string><string>nexus26</string>
...
for other_label in com.trueview26.display com.nexusminimal.display; do
  ...
  if [[ -f "${other_plist}" ]]; then
    /bin/mv -f "${other_plist}" "${other_plist}.disabled-by-nexus26"
  fi
done
if [[ -f "${LEGACY_PLIST}" ]]; then
  /bin/launchctl bootout "gui/$(id -u)/com.antigravity.nexus26" 2>/dev/null || true
...
if [[ "${loaded}" != true ]]; then
  echo "Could not register com.nexus26.display after 5 attempts." >&2
  exit 1
fi
echo "NEXUS 26 installed and started from ${INSTALL_DIR}"
