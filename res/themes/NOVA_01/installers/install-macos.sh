...
INSTALL_DIR="${HOME}/Library/Application Support/NOVA01"
PLIST="${HOME}/Library/LaunchAgents/com.nova01.display.plist"
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.nova01.display</string>
  <key>ProgramArguments</key>
  <array>
    <string>${INSTALL_DIR}/.venv/bin/python</string>
    <string>-m</string><string>nova01</string>
...
/bin/launchctl bootout "gui/$(id -u)/com.nova01.display" 2>/dev/null || true
for other_label in com.trueview26.display com.nexus26.display com.nexusminimal.display; do
  ...
  if [[ -f "${other_plist}" ]]; then
    /bin/mv -f "${other_plist}" "${other_plist}.disabled-by-nova01"
  fi
done
loaded=false
...
if [[ "${loaded}" != true ]]; then
  echo "Could not register com.nova01.display after 5 attempts." >&2
  exit 1
fi
echo "NOVA 01 installed and started from ${INSTALL_DIR}"
